...
from models import db, Patient, Doctor, Department, Appointment, Treatment, ProfilePictures, Shift, Slots

def create_initial_data():
    # don't add data if it already exists
    if Patient.query.first() is not None:
        print("Model code already exists")
        return
    ...
    for d in doctors:
        if d.dept.name == "Pediatrics":
            pediatrics_doctors += [d]
            pd_id += [d.doctor_id]
        else: 
            general_doctors += [d]
            gd_id += [d.doctor_id]
    ...
    for i in range(250):
        ...
        if doctor.doctor_id in pd_id and pd_id != []:  # Pediatric doctors
    ...
    for p in patients:
        ...
        if p.get_age_in_years() < 18:
            ...
            if p.gender == "Female":
                ...
                p.pfp = p3.name
    print("Successfully created Model code")
    db.session.commit()

#-------------------------------------------- Upcoming appointment and slots---------------------------------------------

def add_upcoming_appt():
    patients = Patient.query.all()
    general_patients = []
    pediatrics_patients = []
    for p in patients:
    ...
    # DateTime object for Shift
    date_today = date.today()
    time = datetime.min.time()
    date_today = datetime.combine(date_today, time)

    def make_slots(doctors, shift_list, d):
        check_existing_slots = Slots.query.filter(Slots.date == d).first()
        if check_existing_slots != None:
            print(f"Slots for {d} are already added")
            return 
        
        if len(doctors) == 0:
            print(f"Error making slots for doctor {doctors}")
            return 
        
        # Making slots for all doctors
        for i in range(len(doctors)):
            doctor = doctors[i]

            for l in range(2):
                t3 = random.choice(shift_list)
                shift_list.remove(t3)
            ...
            date_ = d
            time = datetime.min.time()
            date_shift = datetime.combine(date_, time)

            shift_list = Shift.query.filter(Shift.date == date_shift).all()
        db.session.commit()
        return True

    # list of next week dates at a date object: for Slots and Appointments
    next_week_dates_d = [(date.today() + timedelta(days = i)) for i in range(1,8)]
    # list of next week dates at a datetime object: for Shift
    next_week_dates_dt = [(date_today + timedelta(days = i)) for i in range(1,8)]

    today_shift = Shift.query.filter(Shift.date == date_today).all()
    tomorrow_shift = Shift.query.filter(Shift.date == next_week_dates_dt[0]).all()
    day_after_tomorrow_shift = Shift.query.filter(Shift.date == next_week_dates_dt[1]).all()

    make_slots(doctors, today_shift, date.today())
    make_slots(doctors, tomorrow_shift, next_week_dates_d[0])
    make_slots(doctors, day_after_tomorrow_shift, next_week_dates_d[1])

    # Make appointments
    general_doctor_slots_today = Slots.query.filter(Slots.date == date.today(), Slots.doctor_id.in_(gd_id)).all()
    pediatrics_doctor_slots_today = Slots.query.filter(Slots.date == date.today(), Slots.doctor_id.in_(pd_id)).all()

    general_doctor_slots_tommorow = Slots.query.filter(Slots.date == next_week_dates_d[0], Slots.doctor_id.in_(gd_id)).all()
    pediatrics_doctor_slots_tommorow = Slots.query.filter(Slots.date == next_week_dates_d[0], Slots.doctor_id.in_(pd_id)).all()

    general_doctor_slots_day_after_tommorow = Slots.query.filter(Slots.date == next_week_dates_d[1], Slots.doctor_id.in_(gd_id)).all()
    pediatrics_doctor_slots_day_after_tommorow = Slots.query.filter(Slots.date == next_week_dates_d[1], Slots.doctor_id.in_(pd_id)).all()


    def make_appointments(d, patients_list, pediatrics_patients_list, doctor_slots, pediatrics_doctor_slots):
        check_existing_appt = Appointment.query.filter(Appointment.date == d).first()
        if check_existing_appt != None:
            print(f"Appointments for {d} are already added")
            return
        
        if (len(patients_list) == 0 or len(doctor_slots) == 0):
            print(f"Error making appointments for PL, DS")
            return 
        
        
        for i in range(50):
            s = random.choice(doctor_slots)
            index = random.randrange(len(patients_list))
            ...
            if patient_available == []:
                ...
                appointment = Appointment(date = d, start_time = s.start_time, end_time = s.end_time, status = "Booked", 
                                        doctor_id = doctor.doctor_id, patient_id = patient.patient_id, slot_id = s.id)
                db.session.add(appointment)
                doctor_slots.remove(s)
            else:
                pass
        db.session.commit()

        if (len(pediatrics_patients_list) == 0 or len(pediatrics_doctor_slots) == 0):
            print(f"Error making appointments for PPL, PDS")
            return 

        for i in range(20):
            s = random.choice(pediatrics_doctor_slots)
            index = random.randrange(len(pediatrics_patients_list))
            patient = pediatrics_patients_list[index]
            ...
            if patient_available == []:
                ...
                appointment = Appointment(date = d, start_time = s.start_time, end_time = s.end_time, status = "Booked", 
                                        doctor_id = doctor.doctor_id, patient_id = patient.patient_id, slot_id = s.id)
                db.session.add(appointment)
                pediatrics_doctor_slots.remove(s)
            else:
                pass
        db.session.commit()
        return True 

    make_appointments(date.today(), general_patients, pediatrics_patients,  general_doctor_slots_today, pediatrics_doctor_slots_today)
    make_appointments(next_week_dates_d[0], general_patients, pediatrics_patients,  general_doctor_slots_tommorow, pediatrics_doctor_slots_tommorow)
    make_appointments(next_week_dates_d[1], general_patients, pediatrics_patients,  general_doctor_slots_day_after_tommorow, pediatrics_doctor_slots_day_after_tommorow)
